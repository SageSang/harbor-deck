...
const zhCN = {
  ...
  serviceGrid: {
    ...
    confirmDeleteGroupTitle: '删除整个分组？',
    ...
    dragGroup: (name: string) => `拖动分组“${name}”调整顺序`,
    groupOrderUpdated: '分组顺序已更新。',
    groupOrderFailed: '更新分组顺序失败，请稍后再试。',
    selectedCount: (count: number) => `已选择 ${count} 个书签`,
    exitSelection: '退出多选',
    toggleSelection: (name: string) => `选择书签“${name}”`,
    confirmDeleteSelectedTitle: '删除选中的书签？',
    confirmDeleteSelectedMessage: (count: number) =>
      `将从当前场景移除选中的 ${count} 个书签。仍被其他场景引用的书签会保留，没有其他引用的书签会被彻底删除。`,
    deleteSelectedAction: '删除选中书签',
    deleteSelectedActionWithCount: (count: number) => `删除选中的 ${count} 个书签`,
    selectedDeleted: (count: number) => `已删除 ${count} 个书签引用。`,
    dropHint: '拖拽书签到这里即可迁移到当前分组',
    editAction: '修改书签',
    copyLinkAction: '复制链接',
    linkCopied: (name: string) => `已复制“${name}”的链接。`,
    copyLinkFailed: '复制链接失败，请检查浏览器剪贴板权限。',
    ...
    batchAddConfirmMessage: (details: string) =>
      `以下书签已在目标场景的其他分组中：\n${details}\n\n继续后将它们移动到该分组？`,
...
const en: typeof zhCN = {
  ...
  bookmarkManage: {
    ...
    exportSection: {
      ...
      summary:
        'Export the selected scene as an HTML file that Chrome and Edge can import directly.',
  ...
  bookmarkEdit: {
    ...
    createDescription:
      'Create a bookmark in the selected group. You can add more scene placements below.',
  ...
  serviceGrid: {
    ...
    confirmDeleteGroupTitle: 'Delete entire group?',
    confirmDeleteGroupMessage: (name: string, count: number) =>
    ...
    dragGroup: (name: string) => `Drag group “${name}” to reorder`,
    groupOrderUpdated: 'Group order updated.',
    groupOrderFailed: 'Failed to update group order. Please try again later.',
    selectedCount: (count: number) => `${count} bookmark${count === 1 ? '' : 's'} selected`,
    exitSelection: 'Exit selection',
    toggleSelection: (name: string) => `Select bookmark “${name}”`,
    confirmDeleteSelectedTitle: 'Delete selected bookmarks?',
    confirmDeleteSelectedMessage: (count: number) =>
      `This removes the ${count} selected bookmarks from the current scene. Bookmarks referenced by other scenes are kept; bookmarks with no remaining references are deleted permanently.`,
    deleteSelectedAction: 'Delete Selected Bookmarks',
    deleteSelectedActionWithCount: (count: number) =>
      `Delete ${count} Selected Bookmark${count === 1 ? '' : 's'}`,
    selectedDeleted: (count: number) =>
    ...
    linkCopied: (name: string) => `Copied the link for “${name}”.`,
    copyLinkFailed: 'Failed to copy the link. Check browser clipboard permissions.',
    ...
    batchAddDescription:
      'Choose a scene and group for each target scene. Only one group can be selected per scene.',
    batchAddConfirmTitle: 'Confirm group move',
    batchAddConfirmMessage: (details: string) =>
      `These bookmarks already belong to another group in the target scene:\n${details}\n\nContinue and move them to the selected group?`,
  ...
  settings: {
    ...
    webdavSection: {
      ...
      versionsHint: 'Only versions created by HarborDeck under the current WebDAV path are shown.',
